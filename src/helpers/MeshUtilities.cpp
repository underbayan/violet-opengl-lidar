...
void generateMeshFromBuffer() {

}

void parseMesh(mesh_t &mesh,
...
void loadObj(const std::string &filename, mesh_t &mesh, LoadMode mode) {
    ...
    in.close();
    parseMesh(mesh, positions_temp, normals_temp, texcoords_temp, faces_temp, mode);
    positions_temp.clear();
    normals_temp.clear();
    texcoords_temp.clear();
    faces_temp.clear();
    cout << filename << " OBJ: loaded. " << mesh.indices.size() / 3 << " faces, " << mesh.positions.size() <<
    " vertices, " <<
    mesh.normals.size() << " normals, " << mesh.texcoords.size() << " texcoords." << endl;
    return;
}
...
void computeTangentsAndBinormals(mesh_t &mesh) {
    ...
    for (size_t tid = 0; tid < mesh.tangents.size(); ++tid) {
        mesh.tangents[tid] = normalize(
                mesh.tangents[tid] - mesh.normals[tid] * dot(mesh.normals[tid], mesh.tangents[tid]));
